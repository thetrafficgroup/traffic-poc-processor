...
def is_entering_from_outside(line_name, prev_pos, curr_pos, line_coords):
    ...
    curr_cross = cross_product(curr_x, curr_y)
    
    # Definir qué lado es "exterior" para cada línea según la configuración
    # Basado en las coordenadas del lines.json
    if line_name == "NORTE":
        # Línea va de [7,182] a [309,19] - exterior está hacia arriba/izquierda
        return prev_cross > 0  # Viene del lado izquierdo de la línea
    elif line_name == "SUR": 
        # Línea va de [589,383] a [612,51] - exterior está hacia abajo/derecha
        return prev_cross < 0  # Viene del lado derecho de la línea
    elif line_name == "ESTE":
        # Línea va de [616,41] a [312,18] - exterior está hacia la derecha
        return prev_cross < 0  # Viene del lado derecho de la línea
    elif line_name == "OESTE":
        # Línea va de [579,391] a [16,180] - exterior está hacia la izquierda
        return prev_cross > 0  # Viene del lado izquierdo de la línea
...
while cap.isOpened():
                ...
                for line in LINES:
                    ...
                    if crossed and obj_id not in counted_ids_per_line[name]:
                        ...
                        if is_entering_from_outside(name, prev_pos, (cx, cy), line):
                            ...
                            print(f'[✔] ID {obj_id} cruzó {name} (ENTRADA desde afuera)')
                        else:
                            print(f'[✔] ID {obj_id} cruzó {name} (interno, no cuenta para total)')
    ...
    # === Calcular total general (solo vehículos que entraron desde afuera) ===
